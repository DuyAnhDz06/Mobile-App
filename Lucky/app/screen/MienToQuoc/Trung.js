...
import {colors} from '@app/colors/basicColors';
    ...
    <ScrollView>
      <View>
        ...
        <View style={{marginHorizontal: 20}}>
          <TouchableOpacity style={{backgroundColor: '#fff', borderRadius: 25}} onPress={() => setOpen(true)}>
            <Text style={{textAlign: 'center', marginVertical: 10, color: colors.buttons, fontWeight: 'bold', fontSize: 17}}>
        ...
        <View>
          {data.length > 0 &&
            data.map(item => (
              <View>
                {/* {console.log(item)} */}
                <Text style={{textAlign: 'center', marginVertical: 20, fontSize: 20, color: colors.buttons, fontWeight: 'bold'}}>
                  {item.name}
                </Text>
                {item.dataProcess.map(i => {
                  var d = i.result
                    .split(',')
                    .map(e => parseInt(e) % 100)
                    .join(',       ');
                  var e = i.result
                    .split(',')
                    .map(e => parseInt(e) % 1000)
                    .join('      ');
                  return (
                    <View
                      style={{
                        ...
                        borderBottomWidth: 1,
                        borderColor: '#EEEEEE',
                        backgroundColor: i.rank % 2 != 0 ? '#F8F8FF' : 'white',
                      }}>
                      ...
                      <Text
                        style={{
                          ...
                          marginVertical: 5,
                          fontSize: 20,
                          color: i.rank == 'DB' ? colors.buttons : 'black',
                          fontWeight: i.rank == 'DB' ? 'bold' : '400',
                        }}>
                        {type === 'dayDu' ? i.result : type === 'haiSo' ? d : e}
                      </Text>
                    </View>
                  );
                })}
                <View style={{flexDirection: 'row', marginTop: 10, justifyContent: 'space-evenly'}}>
                  <View style={{flexDirection: 'row', alignItems: 'center'}}>
                    <RadioButton
                      ...
                      onPress={() => {
                        setChecked('first'), setType('dayDu');
                      }}
                  ...
                  <View style={{flexDirection: 'row', alignItems: 'center'}}>
                    <RadioButton
                      ...
                      onPress={() => {
                        setChecked('second'), setType('haiSo');
                      }}
                      color={colors.statusbar}
                    />
        ...
        <View style={{marginTop: 15, justifyContent: 'center'}}>
          ...
          <View style={{marginLeft: 5, marginTop: 10}}>
            <Image source={require('@app/assets/images/Trung.png')} style={{width: '100%', height: 200}}></Image>
          </View>
          <TouchableOpacity
            style={{
        ...
        <View>
          <View style={{marginTop: 10}}>
            <Text style={{textAlign: 'center', color: colors.buttons, fontSize: 20, fontWeight: 'bold', marginTop: 15}}>
              Thống kê đầu
            </Text>
          </View>
